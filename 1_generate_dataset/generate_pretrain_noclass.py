...
def main():
    base_dir = "/mnt/data1/zhaoxinjian/zhongkai/TMLR_VISG_/img/all_graphs_noclass"
    graph_types = {
    ...
    for graph_type, generator_func in graph_types.items():
        ...
        for G in graphs:
            # 使用递增编号保存图像
            output_path = os.path.join(base_dir, f"{image_index}.png")
    ...
    # 生成 CSV 文件，只包含图像编号（作为标签）
    data = {'image_index': all_labels}
    df = pd.DataFrame(data)
    df.to_csv(os.path.join(base_dir, "dataset.csv"), index=False)
    
    print("图像生成完成，并生成了包含图像编号的数据集！")
